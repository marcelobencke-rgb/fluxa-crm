...
export type InboxFilter = ConversationStatus | "all" | "unread" | "needs_attention";
...
export function ConversationList({
  ...
  const filtered = useMemo(() => {
    ...
    if (filter === "unread") {
      ...
      result = result.filter((c) => c.needs_attention === true);
    } else if (filter !== "all") {
      result = result.filter((c) => c.status === filter);
    }

    if (assignmentFilter === "mine") {
      result = result.filter((c) => user?.id && c.assigned_agent_id === user.id);
    ...
    return result;
  }, [conversations, filter, assignmentFilter, search, selectedTagIds, selectedCompany, user?.id]);
...
function ConversationItem({
  ...
  const assignedProfile = useMemo(() => {
    ...
    return profiles.find(
      (p) =>
        p.user_id === conversation.assigned_agent_id ||
        p.id === conversation.assigned_agent_id
    );
  }, [conversation.assigned_agent_id, profiles]);

  const assignedName =
    assignedProfile?.full_name || assignedProfile?.email || "Atribuído";

  const contactDeal = contact?.deals?.[0];
  const stageName = contactDeal?.stage?.name || t("noStage");
  const stageColor = contactDeal?.stage?.color || "#64748b";
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
            ...
            <span
              title={
                conversation.assigned_agent_id
                  ? t("assignedTo", { name: assignedName })
                  : t("unassigned")
              }
              className={cn(
        ...
        {/* Badge Row: Needs Attention + Funnel Stage */}
        <div className="flex items-center gap-1.5 mt-1 mb-0.5 flex-wrap">
          {(conversation.needs_attention ||
            ...
            <span
              ...
              className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-red-600 text-white shadow-sm animate-pulse"
            >
