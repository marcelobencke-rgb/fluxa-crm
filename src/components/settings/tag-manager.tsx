...
export function TagManager() {
  ...
  const { tags, loading, mutate } = useTags();
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const { data: freshTags } = await supabase.from('tags').select('*').order('name');
      if (freshTags) mutate(freshTags);
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      mutate(tags.filter((t) => t.id !== tagToDelete.id));
  ...
  return (
    <Card>
      ...
      <ConfirmDialog
        ...
        title={t('deleteTag')}
        description={tagToDelete ? t('deleteConfirm', { name: tagToDelete.name }) : null}
