...
import { ResolveConversationModal } from "./resolve-conversation-modal";
...
export function MessageThread({
  ...
  const handleRemoveAttention = useCallback(async () => {
    if (!conversation) return;
    const supabase = createClient();
    ...
    if (error && error.code === "42703") {
      delete updatePayload.needs_attention;
      await supabase
        .from("conversations")
        .update(updatePayload)
        .eq("id", conversation.id);
    ...
    const agentName =
      ...
      user?.email?.split("@")[0] ||
      "atendente";
    const noteText = `ℹ️ Atenção removida manualmente por ${agentName}.`;
    ...
    // Immediately clear local state flags so badge/tooltip disappear instantly
    conversation.needs_attention = false;
    conversation.ai_autoreply_disabled = false;
    conversation.ai_handoff_summary = null;

    toast.success(t("attentionRemoved"));
    if (onRefresh) onRefresh();
  }, [conversation, t, onRefresh]);
  ...
  const loggedInProfile = profiles.find((p) => p.user_id === user?.id);
  const loggedInAgentName =
    loggedInProfile?.full_name ||
    user?.user_metadata?.full_name ||
    user?.email?.split("@")[0] ||
    "atendente";
    ...
    <div className={cn("flex min-w-0 flex-1 flex-col", DOODLE_BG_CLASSES)}>
      ...
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {/* Session timer badge — Meta Cloud API only */}
          {provider === "meta" && sessionInfo.remaining && (
            <Badge
              variant="outline"
              className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Mais Ações dropdown (Screenshot 2) */}
          <DropdownMenu>
            <DropdownMenuTrigger
              className={cn(
                "inline-flex items-center justify-center h-7 w-7 text-xs rounded-md hover:bg-muted text-muted-foreground",
              )}
              title={t("moreActions")}
            >
            ...
            <DropdownMenuContent
              align="end"
              className="border-border bg-popover min-w-[210px]"
            >
              {(conversation.needs_attention ||
                ...
                >
                  <CircleOff className="mr-2 h-4 w-4 text-muted-foreground" />
                  {t("removeAttention")}
                </DropdownMenuItem>
              )}
              <DropdownMenuItem
                onClick={() => setResolveModalOpen(true)}
                disabled={!(conversation.needs_attention || Boolean(conversation.ai_handoff_summary))}
                className={cn(
                  "text-sm",
                  conversation.needs_attention || Boolean(conversation.ai_handoff_summary)
