      ...
      <Select value={value} onValueChange={(v) => onChange(v || "")}>
        <SelectTrigger className="w-full">
          <SelectValue placeholder={t("tags.select")}>
            {value ? (selected ? selected.name : t("tags.unknown", { id: value })) : t("tags.select")}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="">{t("tags.select")}</SelectItem>
          {tags.map((tg) => (
...
function ContactFieldSelect({
  ...
  return (
    <Select value={value || "name"} onValueChange={(v) => onChange(v || "")}>
      <SelectTrigger className="w-full">
        <SelectValue>
          {(!value || value === "name") ? t("fields.name") :
           value === "email" ? t("fields.email") :
           value === "company" ? t("fields.company") :
           knownCustom ? customFields.find(f => `custom:${f.id}` === value)?.field_name :
...
function AgentSelect({
  ...
  return (
    <Select value={value} onValueChange={(v) => onChange(v || "")}>
      <SelectTrigger className="w-full">
        <SelectValue placeholder={t("agents.select")}>
          {value ? (selected ? (selected.full_name || selected.email || selected.user_id) : t("agents.unknown", { id: value })) : t("agents.select")}
        </SelectValue>
      </SelectTrigger>
      <SelectContent>
        <SelectItem value="">{t("agents.select")}</SelectItem>
        {members.map((m) => (
        ...
        >
          <SelectTrigger className="w-full">
            <SelectValue placeholder={t("pipelines.selectPipeline")}>
              {pipelineId ? (pipelines.find(p => p.id === pipelineId)?.name || t("pipelines.unknownPipeline", { id: pipelineId })) : t("pipelines.selectPipeline")}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
            {pipelines.map((p) => (
      ...
      {pipelineId && (
        <FieldBlock label={t("pipelines.stageLabel")}>
          <Select
            value={stageId}
            onValueChange={(val) =>
              onChange({ pipeline_id: pipelineId, stage_id: val || "" })
            }
          >
            <SelectTrigger className="w-full">
              <SelectValue placeholder={t("pipelines.selectStage")}>
                {stageId ? (stageOptions.find(s => s.id === stageId)?.name || t("pipelines.unknownStage", { id: stageId })) : t("pipelines.selectStage")}
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
              {stageOptions.map((s) => (
      ...
      >
        <SelectTrigger className="w-full">
          <SelectValue placeholder={t("templates.select")}>
            {current ? (hasMatch ? `${templateName} (${language})` : t("templates.unknown", { name: templateName, lang: language || t("templates.unknownLang") })) : t("templates.select")}
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
          <SelectItem value="">{t("templates.select")}</SelectItem>
          {templates.map((tmpl) => {
        ...
        {open && (
          <div className="space-y-3 border-t border-border px-4 py-3">
            <div>
              ...
              <Select
                value={type}
                onValueChange={(val) => onTypeChange(val as AutomationTriggerType)}
              >
                <SelectTrigger className="w-full">
                  <SelectValue>
                    {t(`triggers.${type}.label`)}
    ...
    <div className="space-y-2">
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(val) => onChange({ ...config, match_type: val as "exact" | "contains" })}
        >
          <SelectTrigger className="w-full">
            <SelectValue>
              {config?.match_type === "exact" ? t("config.matchExact") : t("config.matchContains")}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        <>
          <FieldBlock label={t("config.modeLabel")}>
            <Select
              ...
              onValueChange={(val) => set({ mode: val })}
            >
              <SelectTrigger className="w-full">
                <SelectValue>
                  {((cfg.mode as string) ?? "round_robin") === "specific" ? t("config.modes.specific") : t("config.modes.round_robin")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <FieldBlock label={t("config.unitLabel")}>
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue>
                  {((cfg.unit as string) ?? "hours") === "minutes" ? t("config.units.minutes") : ((cfg.unit as string) ?? "hours") === "days" ? t("config.units.days") : t("config.units.hours")}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
        ...
        <>
          <FieldBlock label={t("config.subjectLabel")}>
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue>
                  {t(`config.subjects.${(cfg.subject as string) ?? "tag_presence"}`)}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
