...
describe('buildHandoffSummary', () => {
  it('notes the reply count and quotes the last customer message', () => {
    ...
    expect(summary).toBe(
      '⚠️ Necessita atenção: O agente de IA encaminhou para um atendente após 2 respostas. Motivo / Última mensagem do cliente: “I want a refund”',
    )
  })
  ...
  it('says "without replying" when the bot bailed on the first inbound', () => {
    ...
    expect(summary).toContain('encaminhou para um atendente sem responder.')
  ...
  it('degrades gracefully when there is no customer message', () => {
    ...
    expect(summary).toBe('⚠️ Necessita atenção: O agente de IA encaminhou para um atendente sem responder.')
  })
})
