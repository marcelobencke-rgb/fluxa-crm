...
export async function POST() {
  try {
    ...
    // Evolution API doesn't use Meta's template system — syncing
    // from Meta is not applicable. Return a clear message.
    if ((config.provider ?? 'meta') === 'evolution') {
      return NextResponse.json(
        {
          error:
            'Template sync is only available for Meta Cloud API. Evolution API uses local templates.',
        },
        { status: 400 },
      )
    }
