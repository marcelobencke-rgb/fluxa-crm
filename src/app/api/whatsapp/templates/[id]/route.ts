...
export async function PATCH(
  ...
  try {
    ...
    const isLocal = payload.type === 'local' || existing.meta_template_id.startsWith('local-')
    ...
    // Meta accepted the edit — status flips back to PENDING for review (or APPROVED for local).
...
export async function DELETE(
  ...
  try {
    ...
    const isLocal = existing.meta_template_id?.startsWith('local-')
    if (existing.meta_template_id && !isDryRun() && !isLocal) {
      const { data: config, error: configError } = await supabase
        .from('whatsapp_config')
        .select('*')
        .eq('account_id', accountId)
