...
describe("validateFlowForActivation — nodes", () => {
  ...
  it("flags send_buttons with zero buttons", () => {
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "btn" } },
      {
        node_key: "btn",
        node_type: "send_buttons",
  ...
  it("flags duplicate button reply_ids", () => {
    ...
    expect(
      issues.some((i) => i.message.includes("ID de resposta do botão duplicado")),
    ).toBe(true);
  });
  ...
  it("flags send_list with more than 10 rows total", () => {
    ...
    const nodes = [
      { node_key: "s", node_type: "start", config: { next_node_key: "lst" } },
      {
        node_key: "lst",
        node_type: "send_list",
        config: {
  ...
  it("warns about unreachable nodes", () => {
    const nodes = [
      ...
      { node_key: "island", node_type: "end", config: {} },
    ];
    const issues = validateFlowForActivation(
      { ...validFlow, entry_node_id: "s" },
      nodes,
