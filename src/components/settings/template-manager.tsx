...
export function TemplateManager() {
  ...
  const isMeta = whatsappProvider === 'meta';
  ...
  const [creationTab, setCreationTab] = useState<'meta' | 'local'>('meta');
  ...
  useEffect(() => {
    ...
    // Detect provider from whatsapp_config
    if (accountId) {
      supabase
  ...
  function openEdit(template: MessageTemplate) {
    ...
    setCreationTab(template.meta_template_id?.startsWith('local-') ? 'local' : 'meta');
    setDialogOpen(true);
  }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      toast.success(
        creationTab === 'local'
          ? (isEdit ? 'Modelo local atualizado com sucesso.' : 'Modelo local criado com sucesso.')
          : data.dry_run
            ? isEdit
              ? t('toastSaveEditDry')
              : t('toastSaveNewDry')
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid gap-3 xl:grid-cols-2">
          {templates.map((template) => {
            ...
            const isLocalTemplate = !isMeta || template.meta_template_id?.startsWith('local-');
            return (
                ...
                <CardContent className="flex items-start justify-between pt-4">
                  <div className="space-y-2 min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {template.category && !isLocalTemplate && (
                        <Badge
                      ...
                      {template.language && !isLocalTemplate && (
                        <span className="text-xs text-muted-foreground uppercase">
                          {template.language}
                        </span>
                      )}
                      {!isLocalTemplate && template.quality_score && (
                        <span
                    ...
                    {template.footer_text && !isLocalTemplate && (
                  ...
                  <div className="flex items-center gap-1 shrink-0 ml-2">
                    {(isLocalTemplate || statusKey === 'APPROVED') && (
                      <Button
                        variant="ghost"
                        size="sm"
                    ...
                    {!isLocalTemplate && (statusKey === 'REJECTED' || statusKey === 'PAUSED') && (
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => openEdit(template)}
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              {editingId
                ? creationTab === 'meta'
                  ? t('dialogEditDesc')
                  : 'Edite as informações do modelo local. Estas alterações afetam apenas a Evolution API.'
                : t('dialogNewDesc')}
            </DialogDescription>
          </DialogHeader>

          {!editingId && (
            <Tabs value={creationTab} onValueChange={(val) => setCreationTab(val as 'meta' | 'local')} className="w-full">
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="meta">Meta Cloud API</TabsTrigger>
                <TabsTrigger value="local">Local (Evolution API)</TabsTrigger>
              </TabsList>
            </Tabs>
          )}

          <div className="space-y-6 py-4">
            {creationTab === 'meta' && form.category === 'Authentication' && (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent className="bg-popover border-border">
                      <SelectItem value="pt_BR" className="text-popover-foreground focus:bg-muted focus:text-popover-foreground">pt_BR (Português)</SelectItem>
                      <SelectItem value="en_US" className="text-popover-foreground focus:bg-muted focus:text-popover-foreground">en_US (Inglês)</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-[11px] text-muted-foreground">
                    {editingId ? (
            ...
            <div className="space-y-2">
              ...
              {creationTab === 'meta' && bodyVarCount > 0 && (
                        ...
                        >
                          ...
                          {btn.type === 'URL' && (
                            <div className="space-y-1 pl-1">
                              ...
                              />
                              {creationTab === 'meta' && extractVariableIndices(btn.url).length > 0 && (
                                <Input
          ...
          <DialogFooter className="bg-popover border-border">
            ...
            >
              {submitting ? (
                <>
                  <Loader2 className="size-4 animate-spin" />
                  {editingId ? t('saving') : (creationTab === 'meta' ? t('submitting') : t('saving'))}
                </>
              ) : editingId ? (
                creationTab === 'meta' ? t('saveResubmit') : 'Salvar Alterações'
              ) : (
                creationTab === 'meta' ? t('submitApproval') : 'Salvar Modelo'
              )}
            </Button>
          </DialogFooter>
          </form>
