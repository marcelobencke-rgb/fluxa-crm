...
//                        configured provider (Meta or Evolution,
...
import { resolveTemplateText } from '@/lib/whatsapp/template-resolver';
...
export interface BroadcastPlan {
  ...
  /** Which provider to use for delivery. */
  providerType: 'meta' | 'evolution';
  /** Meta-specific — phone number ID for Cloud API. */
  phoneNumberId: string;
  /** Meta-specific — decrypted access token. */
  accessToken: string;
...
export async function createBroadcast(
  ...
  const providerType: 'meta' | 'evolution' = config.provider === 'evolution' ? 'evolution' : 'meta';

  let accessToken = '';
  if (providerType === 'meta') {
    if (!config.access_token) {
      throw new BroadcastError('config_missing', 'Meta Cloud API requires an Access Token.', 400);
    }
    accessToken = decrypt(config.access_token);
  }

  if (providerType === 'evolution') {
    if (!config.evolution_instance_id || !config.evolution_api_url || !config.evolution_api_key) {
      throw new BroadcastError(
        'config_missing',
  ...
  return {
    ...
    evolutionInstanceId: config.evolution_instance_id ?? '',
    templateRow,
    planned,
    rejected,
  };
...
export async function deliverBroadcast(
  ...
  if (plan.providerType === 'evolution') {
    provider = new EvolutionProvider(
      plan.evolutionApiUrl,
      plan.evolutionApiKey,
      plan.evolutionInstanceId,
    );
  } else {
    provider = new MetaProvider(plan.phoneNumberId, plan.accessToken, {
      fetchTemplateRow: async () => plan.templateRow,
    });
  }
  ...
  for (const recipient of plan.planned) {
    ...
    for (const variant of variants) {
      try {
        // For Evolution: resolve template text locally with variables;
        // for Meta: the MetaProvider handles native template send.
        const templateMessageParams = plan.providerType === 'evolution' && plan.templateRow
          ? {
