...
export function MessageComposer({
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!trimmed || sending || (!isNoteMode && sessionExpired)) return;

    setSending(true);
    try {
      onSend(trimmed, replyTo?.id, isNoteMode);
      setText("");
  ...
  }, [text, sending, sessionExpired, isNoteMode, onSend, replyTo?.id]);
      ...
      {draft ? (
        ...
        <div className="flex items-end gap-2">
          ...
          <GatedButton
            ...
            canAct={!readOnly}
            gateReason="send messages"
            title={isNoteMode ? "Alternar para mensagem WhatsApp" : "Alternar para Nota Interna (visível apenas para a equipe)"}
            className={cn(
              "h-9 w-9 shrink-0 p-0 transition-colors",
              isNoteMode
                ? "bg-amber-500/20 text-amber-500 hover:bg-amber-500/30 hover:text-amber-400"
          ...
          <textarea
            ...
            disabled={(!isNoteMode && sessionExpired) || readOnly}
            rows={1}
            ...
            className={cn(
              ...
              isNoteMode && "border-amber-500/60 bg-amber-500/15 focus:border-amber-400 placeholder-amber-800/70 dark:placeholder-amber-300/60 text-amber-950 dark:text-amber-100 font-medium shadow-inner",
              ((!isNoteMode && sessionExpired) || readOnly) && "cursor-not-allowed opacity-50"
            )}
          />
          ...
          <GatedButton
            ...
            canAct={!readOnly}
            gateReason="send messages"
            disabled={!text.trim() || (!isNoteMode && sessionExpired) || sending}
            onClick={handleSend}
            className={cn(
