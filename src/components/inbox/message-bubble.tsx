...
function StatusIcon({ status }: { status: Message["status"] }) {
  switch (status) {
    ...
    case "read":
      return <CheckCheck className="h-3.5 w-3.5 text-sky-300 drop-shadow-xs font-bold" />;
    case "failed":
...
function MessageContent({ message, t }: { message: Message, t: ReturnType<typeof useTranslations> }) {
  switch (message.content_type) {
    case "text":
      return (
        <p className={cn("whitespace-pre-wrap break-words text-sm", message.is_internal && "text-amber-950 dark:text-amber-100 font-medium")}>
    ...
    >
      <div
        className={cn(
          ...
          isInternal
            ? "border border-amber-500/40 bg-amber-500/10 text-amber-950 dark:text-amber-100 rounded-xl rounded-br-md"
            : isAgent
      ...
      >
        {isInternal && (
          <div className="mb-1 flex items-center justify-between gap-2 border-b border-amber-500/20 pb-1 text-[11px] font-semibold text-amber-600 dark:text-amber-400">
        ...
        <div
          className={cn(
            ...
            isInternal ? "justify-end" : isAgent ? "justify-end" : "justify-start",
          )}
        >
          {/* AI badge — on replies the auto-reply bot generated or AI handoff notes */}
