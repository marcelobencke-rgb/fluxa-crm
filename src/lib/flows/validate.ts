    ...
    if (!keywords || keywords.length === 0) {
      ...
      if (blanks > 0) {
        issues.push({
          ...
          scope: "trigger",
          field: "trigger_config.keywords",
          message: `${blanks} palavra(s)-chave em branco — elas não vão corresponder a nada.`,
        });
      }
    }
  }
    ...
    case "send_message": {
      ...
      if (!cfg.text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "text",
          message: "O nó Enviar mensagem precisa de um texto.",
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "O nó Enviar mensagem deve apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó Enviar mensagem aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "send_media": {
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "media_type",
          message: "O nó Enviar mídia precisa de um tipo (imagem, vídeo ou documento).",
        });
      }
      if (!cfg.media_url?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "media_url",
          message: "O nó Enviar mídia precisa de um arquivo (faça upload de um antes de ativar).",
        });
      }
      ...
      if (!cfg.next_node_key) {
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `O nó Enviar mídia aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "send_buttons": {
      ...
      if (!cfg.text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "text",
          message: "O nó Enviar botões precisa de um texto.",
        });
      }
      ...
      btns.forEach((b, i) => {
        ...
        if (!b.reply_id?.trim()) {
          ...
          issues.push({
            ...
            node_key: node.node_key,
            field: `${field}.reply_id`,
            message: `ID de resposta do botão duplicado "${b.reply_id}".`,
          });
        }
    ...
    case "send_list": {
      ...
      if (!cfg.text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "text",
          message: "O nó Enviar lista precisa de um texto.",
        });
      }
      if (!cfg.button_label?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "button_label",
          message: "Enviar lista precisa de um rótulo de botão (texto para expandir).",
        });
      }
        ...
        rows.forEach((row, ri) => {
          ...
          if (!row.reply_id?.trim()) {
            ...
            issues.push({
              ...
              node_key: node.node_key,
              field: `${field}.reply_id`,
              message: `ID da linha da lista duplicado "${row.reply_id}".`,
            });
          }
    ...
    case "collect_input": {
      ...
      if (!cfg.prompt_text?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "prompt_text",
          message: "Coletar entrada precisa de uma pergunta para enviar ao cliente.",
        });
      }
      if (!cfg.var_key?.trim()) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: "Coletar entrada precisa de uma chave de variável (var_key) para salvar a resposta.",
        });
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "var_key",
          message: `A chave da variável "${cfg.var_key}" deve ser alfanumérica + sublinhado e começar com uma letra ou sublinhado.`,
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "Coletar entrada deve apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `Coletar entrada aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
    ...
    case "condition": {
      ...
      if (!cfg.subject || !["var", "tag", "contact_field"].includes(cfg.subject)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "subject",
          message: "A Condição precisa de um sujeito (variável, tag, campo do contato).",
        });
      }
      ...
      if (
        ...
        issues.push({
          ...
          node_key: node.node_key,
          field: "value",
          message: `O operador "${cfg.operator}" geralmente espera um valor de comparação — o valor vazio só fará correspondência com sujeitos vazios.`,
        });
      }
      for (const branch of ["true_next", "false_next"] as const) {
        ...
        if (!key) {
          issues.push({
            ...
            node_key: node.node_key,
            field: branch,
            message: `A condição precisa de um nó para a ramificação "${branch === "true_next" ? "verdadeira" : "falsa"}".`,
          });
          ...
          issues.push({
            ...
            node_key: node.node_key,
            field: branch,
            message: `A ramificação "${branch}" da Condição aponta para um nó inexistente "${key}".`,
          });
        }
      }
      break;
    ...
    case "set_tag": {
      ...
      if (!cfg.mode || !["add", "remove"].includes(cfg.mode)) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "mode",
          message: "Adicionar Tag precisa de um modo (adicionar ou remover).",
        });
      }
      if (!cfg.tag_id) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "tag_id",
          message: "Adicionar Tag precisa de uma tag para aplicar.",
        });
      }
      if (!cfg.next_node_key) {
        issues.push({
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: "Adicionar Tag deve apontar para um próximo nó.",
        });
      } else if (!knownKeys.has(cfg.next_node_key)) {
        issues.push({
          severity: "error",
          ...
          node_key: node.node_key,
          field: "next_node_key",
          message: `Adicionar Tag aponta para um nó inexistente "${cfg.next_node_key}".`,
        });
      }
      break;
    }
