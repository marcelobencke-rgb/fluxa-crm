...
export async function POST(request: Request) {
  try {
    ...
    // Load WhatsApp config to detect provider. Needed both for the
    // Evolution short-circuit and for the Meta submission path.
    ...
    if (payload.type === 'local') {
      // Local templates are approved instantly and bypass Meta validation.
      metaTemplateId = `local-${crypto.randomUUID()}`
      metaStatus = 'APPROVED'
    } else if (dryRun) {
