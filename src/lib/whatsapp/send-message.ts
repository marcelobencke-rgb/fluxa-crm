...
import type { IWhatsAppProvider } from './providers/types';
...
export interface SendMessageParams {
  ...
  /** True when creating an internal note (not sent via WhatsApp). */
  isInternal?: boolean;
}
...
export async function sendMessageToConversation(
  ...
  const { data: config, error: configError } = await db
    ...
    .select('phone_number_id, access_token, status, provider, evolution_instance_id, evolution_api_url, evolution_api_key, id')
    .eq('account_id', accountId)
    .maybeSingle();
  ...
  if (config.provider === 'meta' && !config.phone_number_id) {
    throw new SendMessageError(
      'config_missing',
      'Meta Cloud API requires a Phone Number ID. Please configure it in settings.',
      400
    );
  }

  if (config.provider === 'evolution' && (!config.evolution_instance_id || !config.evolution_api_url || !config.evolution_api_key)) {
    throw new SendMessageError(
      'config_missing',
      'Evolution API requires Instance ID, URL, and API Key. Please configure them in settings.',
      400
    );
  }
  ...
  if (config.provider === 'meta') {
    if (!config.access_token) {
      throw new SendMessageError(
        'config_missing',
        'Meta Cloud API requires an Access Token.',
        400
      );
    }
    // Access token is encrypted. If this throws, the key rotated and broke it.
    try {
      accessToken = decrypt(config.access_token);
    } catch (err) {
      console.error('[send-message] failed to decrypt access_token:', err);
      throw new SendMessageError('config_corrupt', 'Invalid access token format.', 500);
    }
  }

  // Self-heal legacy CBC ciphertexts. Fire-and-forget; idempotent.
  if (config.provider === 'meta' && isLegacyFormat(config.access_token)) {
    void db
      .from('whatsapp_config')
  ...
  if (config.provider === 'evolution') {
    ...
    provider = new MetaProvider(config.phone_number_id, accessToken, {
      fetchTemplateRow: async () => templateRow,
    });
  }
  ...
  if (params.isInternal) {
    // Internal notes are staff-only and bypass WhatsApp providers
    waMessageId = `note_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`;
  } else {
    // Send via Meta/Evolution — retry across phone-number variants if Meta rejects
    // with "recipient not in allowed list"; persist a working variant
    // back to the contact so the next send goes straight through.
    try {
